...
(defun read-sring-posframe--read-string ( PROMPT &optional INITIAL-INPUT HISTORY DEFAULT-VALUE INHERIT-INPUT-METHOD KEYMAP)
  (advice-remove 'read-from-minibuffer #'read-sring-posframe--read-from-minibuffer)
...
(defun read-sring-posframe--read-from-minibuffer (prompt &optional initial keymap read history default inherit-input-method)
  (read-sring-posframe--read-string prompt initial history default inherit-input-method keymap))

(defun read-string-posframe--replace-read-from-minibuffer (orig-fn &rest args)
  (advice-add 'read-from-minibuffer :override #'read-sring-posframe--read-from-minibuffer)
  (unwind-protect
      (apply orig-fn args)
    (advice-remove 'read-from-minibuffer #'read-sring-posframe--read-from-minibuffer)))
...
(define-minor-mode read-string-posframe-mode
  ...
  (if read-string-posframe-mode
      (progn
        ...
        (advice-add 'read-string :override #'+my-read-string))
    (progn
